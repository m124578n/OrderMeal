...
def get_all_groups_by_status(request, *args, **kwargs):
    ...
    # 排除自己已經有購買的團
    # 找到自己所在的團
    group_member = GroupMember.objects.select_related("group").only("group__id").filter(group_member__staff_id=user_id, group__status=Group.Status.START)
    ...
    # 輸出需要顯示的欄位
    groups = groups.values_list("id", "status", "note", "store")
    return JsonResponse({"status" : 200, "groups" : list(groups)})

@verify_jwt
def get_groups_by_user_id(request, *args, **kwargs):
    user_id = kwargs["user_id"]
    groups = GroupMember.objects.select_related("group").only("group__id", "group__status", "group__note").filter(group_member__staff_id=user_id, group__status=Group.Status.START)
    groups = groups.values_list("group__id", "group__status", "group__note", "group__store")
    return JsonResponse({"status" : 200, "groups" : list(groups)})
